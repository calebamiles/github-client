...
var _ = Describe("FetchCommitsToPathSince", func() {
	It("returns a slice of commits with comments", func() {
		...
		fetcher.FetchReturns(pullRequestPagesStub, nil)
		...
		commit := commits[0]
		Expect(commit.Comments()).ToNot(BeEmpty())
	})
	...
	It("passes the correct URL to the fetcher", func() {
		...
		query := u.Query()
		Expect(query.Get("per_page")).To(Equal(internal.NumberOfPagesToRequest)) // fairly brittle test
...
var pullRequestPagesStub = [][]byte{[]byte(pullRequetsStub)}

const pullRequetsStub = `
[
  {
