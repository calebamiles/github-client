...
var _ = Describe("JsonPageJoiner", func() {
	Describe("Join", func() {
		Context("when given an empty slice", func() {
			It("returns an empty slice", func() {
				var noPages [][]byte
				Expect(pages.Join(noPages)).To(HaveLen(0))
			})
		})

		Context("when given a single page", func() {
			It("returns a combined slice of JSON", func() {
				singlePage := [][]byte{[]byte(pullsStub)}
				combinedPages := pages.Join(singlePage)

				var unmarshalTarget []interface{}
				err := json.Unmarshal(combinedPages, &unmarshalTarget)
				Expect(err).ToNot(HaveOccurred())
				Expect(unmarshalTarget).To(HaveLen(1))
			})
		})

		Context("when given two pages", func() {
			It("returns a combined slice of JSON", func() {
				singlePage := [][]byte{[]byte(pullsStub), []byte(pullsStub)}
				combinedPages := pages.Join(singlePage)

				var unmarshalTarget []interface{}
				err := json.Unmarshal(combinedPages, &unmarshalTarget)
				Expect(err).ToNot(HaveOccurred())
				Expect(unmarshalTarget).To(HaveLen(2))
			})
		})

		Context("when given more than two pages", func() {
			It("returns a combined slice of JSON", func() {
				singlePage := [][]byte{[]byte(pullsStub), []byte(pullsStub), []byte(pullsStub)}
				combinedPages := pages.Join(singlePage)

				var unmarshalTarget []interface{}
				err := json.Unmarshal(combinedPages, &unmarshalTarget)
				Expect(err).ToNot(HaveOccurred())
				Expect(unmarshalTarget).To(HaveLen(3))
			})
		})
	})
